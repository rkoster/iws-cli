...
// LaunchInstance creates and starts a new Incus instance
func (c *Client) LaunchInstance(imageRef, instanceName, pool, network string) error {
	...
	// First try to resolve alias to fingerprint
	aliasEntry, _, err := targetClient.GetImageAlias(alias)
	if err == nil {
		// Alias found, use the fingerprint
		imageInfo, _, err = targetClient.GetImage(aliasEntry.Target)
		if err != nil {
			return fmt.Errorf("failed to get image by fingerprint '%s': %w", aliasEntry.Target, err)
		}
	} else {
		// Not an alias, try as fingerprint directly
		imageInfo, _, err = targetClient.GetImage(alias)
		if err != nil {
			return fmt.Errorf("failed to get image '%s': %w", alias, err)
		}
	}

	// Create instance from image with config in the request
	req := api.InstancesPost{
		Name:   instanceName,
		Type:   api.InstanceTypeContainer,
		Source: api.InstanceSource{
			Type:        "image",
			Fingerprint: imageInfo.Fingerprint,
		},
	}
	...
	// Use CreateInstanceFromImage which handles everything properly
	op, err := targetClient.CreateInstanceFromImage(targetClient, *imageInfo, req)
	if err != nil {
	...
	// Start the instance if the server doesn't support auto-start
	if !targetClient.HasExtension("instance_create_start") {
		reqState := api.InstanceStatePut{
			Action:  "start",
			Timeout: -1,
		}

		_, err = targetClient.UpdateInstanceState(instanceName, reqState, "")
		if err != nil {
			return fmt.Errorf("failed to start instance: %w", err)
		}
	} else {
		// Even if the extension is supported, verify the instance is running
		running, err := c.IsInstanceRunning(instanceName)
		if err != nil {
...
func (c *Client) parseImageRef(ref string) api.InstanceSource {
	...
	// Check if it's a local alias (starts with /)
	if len(ref) > 0 && ref[0] == '/' {
		source.Alias = ref[1:]
	} else if len(ref) > 0 && ref[0] == ':' {
		// Remote alias without server
		source.Alias = ref[1:]
	} else {
		// Check if it's a local alias or a full reference
		parts := splitImageRef(ref)
		if len(parts) == 2 {
			// Check if the first part is a known remote and different from the target server
			if _, exists := c.Config.Remotes[parts[0]]; exists && parts[0] != c.RemoteName {
				source.Server = parts[0]
				source.Alias = parts[1]
			} else {
				// It's a local alias or the same server
				source.Alias = ref
			}
		} else {
			source.Alias = ref
